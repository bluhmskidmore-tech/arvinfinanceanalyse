...
项目现存两套正式视觉语言，**一页一主题，页内禁止混用或中途翻转**（Page Theme Lock）。唯一书面豁免见 §2.4。
...
用于高密度决策页：经营日报首页（`dhApiBackedHome`）、组合首页、债券分析驾驶舱、股票分析。近黑藏青底 + 分层面板 + **去饱和**冷强调。

> **宏观工具箱**不在深色终端名单内：该页为工具表单型，保持 **IB 浅色**（业主 2026-07-19 拍板，与提案 D1-b 一致）。
...
### 2.4 豁免：市场数据页 command sheet 深色岛（2026-07-19）

| 项 | 约定 |
|---|---|
| **路由** | `/market-data` 仅此一路由 |
| **范围** | 首屏 overview **command sheet**（含其深色头栏 / tape 列头等与 command 一体的深色扫描区）；页面其余主体仍为 **IB 浅色** |
| **Token** | 深色岛必须收敛到 `--dh-api-*`（圆角 `6px`）；禁止继续扩张自制 `--command-*` / 裸 navy hex 作为第三套主题 |
| **禁止扩散** | 不得以此为由在其他浅色页再插深色区块；不得把整页 `/market-data` 改成深色终端 |
| **审查口径** | Anti-pattern §11.1 对本豁免范围不记一票否决；豁免区外的页内翻转仍否决 |

业主拍板：保留交易终端感首屏深色岛，不强制改浅色 IB。
...
1. 页内主题翻转（浅色页中插深色区块，反之亦然）——**§2.4 已豁免的 `/market-data` command sheet 深色岛除外**

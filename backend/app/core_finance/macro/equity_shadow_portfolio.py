...
import duckdb
import pandas as pd
from backend.app.core_finance.factor_screen_candidates import MAX_CANDIDATES, TOP_PCT
...
def _duckdb_open_warnings(exc: duckdb.Error) -> list[str]:
    ...
    if any(marker in message for marker in DUCKDB_BUSY_MARKERS):
...
def _filter_factor_screen_universe(frame: pd.DataFrame) -> pd.DataFrame:
    stock_name = frame.get("stock_name", pd.Series("", index=frame.index)).fillna("").astype(str).str.upper()
    mask = ~stock_name.str.match(r"^\*?ST")
    mask &= pd.to_numeric(frame["roe"], errors="coerce").abs() <= MAX_ABS_ROE
    mask &= pd.to_numeric(frame["dividend_yield"], errors="coerce") <= MAX_DIVIDEND_YIELD
    mask &= pd.to_numeric(frame["gross_margin"], errors="coerce") >= MIN_POSITIVE_MARGIN
    return frame[mask]


def _industry_neutralize_factors(factors: pd.DataFrame, industries: pd.Series) -> pd.DataFrame:
    neutralized = factors.copy()
    aligned_industries = industries.reindex(factors.index).astype(str)
    for column in neutralized.columns:
        industry_mean = neutralized[column].groupby(aligned_industries).transform("mean")
        neutralized[column] = neutralized[column] - industry_mean
    return neutralized
...
def _benchmark_result(
    ...
    for period in periods:
        returns = returns_by_period.get(period, pd.Series(dtype="float64"))
...
def _shadow_admission(
    ...
    criteria = [
        _admission_criterion("history_length", completed_periods >= ADMISSION_MIN_PERIODS, completed_periods, f">={ADMISSION_MIN_PERIODS}"),
        ...
        _admission_criterion("blocking_warnings", not blocking_warnings, blocking_warnings, "none"),
    ]
    ...
    if hard_failed:
        status = "failed"
        label = "Failed"
        summary = "Not ready for production promotion"
    elif needs_review:
        status = "needs_review"
        label = "Needs review"
        ...
        status = "passed"
        label = "Passed"
        summary = "Eligible for production-rule candidate review"
    return {"status": status, "label": label, "summary": summary, "criteria": criteria}
...
def _cost_gate(reference: dict[str, object], candidate: dict[str, object], cost_bps: int) -> dict[str, object] | None:
    ...
    return {
        "passed": passed,
        "actual": {"total_return": _round(candidate_total), "excess_return": _round(candidate_excess)},
        "threshold": {"total_return": f">{_round(reference_total)}", "excess_return": f">{_round(reference_excess)}"},
    }
...
def _admission_criterion(key: str, passed: bool, actual: object, threshold: object) -> dict[str, object]:
    return {"key": key, "passed": passed, "actual": actual, "threshold": threshold}

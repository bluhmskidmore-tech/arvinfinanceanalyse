...
QDB_FINANCIAL_INDICATOR_SOURCE = "QDB源分析：总账对账+日均可复算"
FORMAL_FINANCIAL_INDICATOR_PENDING_SOURCE = (
    "formal_pending: 正式财务指标依赖合并范围、拨备/核销/收回、子公司抵消或管理层调整来源，当前未接入"
)
FORMAL_FINANCIAL_INDICATOR_PENDING_NAMES = [
    "集团营业收入",
    "业务及管理费",
    "减值损失",
    "利润总额",
    ...
    *,
    ...
    sheets = [
        ...
        _sheet(
            ...
            "财务指标落地状态",
            ["指标", "当前值", "单位", "口径状态", "口径来源"],
            _financial_indicator_status_rows(metrics),
        ),
...
def _financial_indicator_status_rows(metrics: dict[str, Decimal]) -> list[dict[str, Any]]:
    qdb_rows = [
        ...
        ("贷款总额（QDB源）", _display_number(_to_yi(metrics["贷款总额"])), "亿元"),
        ("存款总额（QDB源）", _display_number(_to_yi(metrics["存款总额"])), "亿元"),
        ("投资总额（QDB源）", _display_number(_to_yi(metrics["投资总额"])), "亿元"),
        ("存贷比（QDB源）", _display_number(metrics["存贷比%"]), "%"),
        ("贷款减值准备率（QDB源）", _display_number(metrics["贷款减值准备率%"]), "%"),
        ("定期化率（QDB源）", _display_number(metrics["定期化率%"]), "%"),
        ("活期率（QDB源）", _display_number(metrics["活期率%"]), "%"),
        ("高流动性占比（QDB源）", _display_number(metrics["高流动性占比%"]), "%"),
    ]
    rows = [
        {
            "指标": name,
            ...
            "单位": unit,
            "口径状态": "QDB源可复算",
            "口径来源": QDB_FINANCIAL_INDICATOR_SOURCE,
        }
        for name, value, unit in qdb_rows
    ]
    rows.extend(
        {
            "指标": name,
            ...
            "单位": "待确认",
            "口径状态": "正式口径待接入",
            "口径来源": FORMAL_FINANCIAL_INDICATOR_PENDING_SOURCE,
        }
        for name in FORMAL_FINANCIAL_INDICATOR_PENDING_NAMES
    )
    return rows

...
PNL_MODULE = ensure_formal_module(
    FormalComputeModuleDescriptor(
        module_name="pnl",
        basis="formal",
        input_sources=("fi_pnl_record", "nonstd_journal_entries"),
...
PNL_FORMAL_BASIS = PNL_MODULE.basis
CACHE_KEY = PNL_MODULE.cache_key
PNL_MATERIALIZE_LOCK = PNL_MODULE.lock_definition
# API result_meta.cache_version: formal basis + materialize rule bundle (distinct from scenario/analytical).
PNL_RESULT_CACHE_VERSION = PNL_MODULE.cache_version


def _execute_pnl_materialization(
    *,
    report_date: str,
    is_month_end: bool,
    ...
    _assert_partition_matches(
        report_date=target_report_date,
        fi_rows=normalized_fi,
        nonstd_rows=normalized_nonstd,
    )
...
def _materialize_pnl_facts(
    *,
    ...
    return run_formal_materialize(
        descriptor=PNL_MODULE,
        job_name="pnl_materialize",
        report_date=report_date,
        governance_dir=str(governance_path),
        lock_base_dir=str(duckdb_file.parent),
        run_id=run_id,
        execute_materialization=lambda: _execute_pnl_materialization(
            report_date=report_date,
            is_month_end=is_month_end,
            fi_rows=fi_rows,
            nonstd_rows_by_type=nonstd_rows_by_type,
            duckdb_file=duckdb_file,
            formal_pnl_enabled=formal_pnl_enabled,
            formal_pnl_scope_json=formal_pnl_scope_json,
        ),
    )
...
def _run_pnl_materialize_sync(**kwargs) -> dict[str, object]:
    payload = _materialize_pnl_facts(**kwargs)
    return {
        key: value
        for key, value in payload.items()
        if key != "payload" and key != "cache_version"
    }


run_pnl_materialize_sync = _run_pnl_materialize_sync
    ...
    *,
    ...
    for row in fi_rows:
        if row.report_date != report_date:
            raise ValueError(
                f"FI row report_date {row.report_date.isoformat()} does not match task report_date {report_date.isoformat()}"
            )

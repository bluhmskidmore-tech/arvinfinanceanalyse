...
- 10 个计算/展示 P1 仍未取得 owner 口径裁决；P1-08 债券看板缺失值/假环比已由 formatter 与页面测试证据关闭。
...
| 3 | Credit spread rating-tenor matrix | 评级-期限矩阵由后端治理，还是前端硬编码聚合？ | 建议后端提供治理矩阵。 |
...
| 6 | Period yield denominator | 季度/年度收益率分母用期间平均规模、月末快照求和，还是日均？ | 建议避免月末快照求和作为规模口径。 |

...
| 输出项 | 必填内容 | 不可替代项 |
| --- | --- | --- |
| 10 个剩余计算/展示 P1 裁决 | 每个剩余 P1 选择一个权威口径，或要求补充源数据证据 | 不能用当前测试通过替代口径裁决；P1-08 已有工程验证证据，不再占用 owner 口径会 |
...
| 真实后端 smoke 安排 | live backend 目标、数据源、执行窗口、失败处理人 | mock smoke 不是 full real-backend smoke |
...
- 10 个计算/展示 P1 仍未取得 owner 口径裁决；P1-08 债券看板缺失值/假环比已由 formatter 与页面测试关闭。
- full real-backend browser smoke 还未完成。
- direct Codex App MCP / GitNexus evidence 还未在当前工具面复核。

## 先裁决的 10 个剩余 P1

| 顺序 | P1 | Owner 需要回答的问题 | 推荐会议处理 |
| --- | --- | --- | --- |
| 1 | Balance movement share source | 当前余额占比以后端为准，还是前端可重算？ | 建议以后端 DTO 为准；前端只展示。 |
| 2 | Frontend formal aggregation | 正式 PnL、收益率、ADB 聚合能否在前端补算？ | 建议正式指标只由后端提供，前端只做非正式 UI 辅助。 |
| 3 | Credit spread rating-tenor matrix | 评级-期限桶由后端治理，还是前端硬编码聚合？ | 建议后端提供治理矩阵。 |
| 4 | Bond analytics rate unit | `ytm_value` 和票息率的权威单位是什么？ | 建议源数据显式携带单位，正式路径移除启发式。 |
| 5 | Campisi coupon income | `coupon_rate` 存储和计算单位是小数还是百分数？ | 建议先用真实 ZQTZ 数据确认，再统一一个 rate-unit helper。 |
| 6 | Period yield denominator | 季度/年度收益率分母用期间平均规模、月末快照求和，还是日均？ | 建议避免月末快照求和作规模。 |
| 7 | Roll-down sign | 曲线 roll-down 采用哪个业务符号约定？ | 建议写入 `docs/calc_rules.md` 后统一实现。 |
| 8 | PnL bridge zero-actual residual | `actual_pnl=0` 且解释项非零时应 warning/undefined 还是 ok？ | 建议 fail-loud，不能隐藏 residual。 |
| 9 | Macro liquidity score polarity | 流动性分数是宽松为正，还是收紧压力为正？ | 建议明确所有 composite component 的同向性。 |
| 10 | QDB position-vs-ledger reconciliation | 当前同源比较是控制项还是诊断项？ | 建议若声称控制，必须使用独立 position 和 ledger source anchor。 |
...
7. live backend 准备好后跑 full real-backend smoke。

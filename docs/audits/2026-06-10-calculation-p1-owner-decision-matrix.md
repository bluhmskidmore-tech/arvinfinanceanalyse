...
**2026-07-16 P1-09 overlay**: user/chat decision selected Option A, making backend `current_balance_pct` authoritative. The frontend now consumes only that governed value; a missing or invalid backend share remains missing, renders as `—`, and suppresses the structure chart instead of recomputing from visible balances or showing 0%. The authoritative display contract is recorded as `MTR-BMV-005` in `docs/metric_dictionary.md`. The 2026-06-10 JSON snapshot and generated intake packets remain historical and were not regenerated; this note plus the capture-template row are the human-readable closure overlay for P1-09 and do not certify page business-owner approval.
...
| P1-07 | Macro liquidity score polarity | Selected 2026-07-03: composite stays bond-unfavorable/tightness-positive while `liquidity_score` stays looseness-positive. | A: all components tightness-positive; B: liquidity remains looseness-positive but enters composite with inverse sign; C: separate liquidity narrative from composite. | Implemented Option B as `0.4*rate - 0.3*liquidity + 0.2*growth + 0.1*inflation`; `build_macro_context_v1` exposes score polarity and formula metadata. | Closed for the "funding looseness increases tightening / shorten duration" defect. | Isolated liquidity regression in `tests/test_macro_bond_linkage.py`; macro-context metadata assertions; frontend copy states polarity. |
...
- After the 2026-07-16 P1-09 overlay, rows still needing owner-decision work are: `P1-01`, `P1-02`, `P1-03`, `P1-04`, `P1-05`, `P1-06`, `P1-10`, `P1-11`.
...
- **P1-09 Balance movement share source**: completed 2026-07-16 under Option A in `frontend/src/features/balance-movement-analysis/pages/BalanceMovementAnalysisPage.tsx`; keep `current_balance_pct` backend-authoritative, preserve missing/invalid shares as missing, and retain `frontend/src/test/BalanceMovementAnalysisPage.test.tsx` before changing this boundary.

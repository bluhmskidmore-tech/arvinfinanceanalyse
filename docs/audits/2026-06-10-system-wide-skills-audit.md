...
- `python scripts\system_audit_pulse.py --generated-at 2026-08-06T22:30:00+08:00` -> expected non-zero with `status=fail`, `completion_state=not_complete`, `open_blocker_count=5`, `route_count=40`, `visible_unseeded_route_count=1`, `business_contract_certified_count=0`, `tracked_route_count=26`, `route_gap_count=0`, completion snapshot status fail/error count 2, strict-matrix status fail, and `drift_error_count=6`; `--format markdown` renders the same non-approval watch board for human review. This is monitoring evidence only, not completion evidence.
...
- `docs/audits/2026-06-10-system-audit-pulse-snapshot.json` records the latest saved read-only pulse: status fail, completion state not complete, 5 open blockers, 40 route-scope routes with 1 visible-unseeded route, 26 tracked business-display routes, 0 route gaps, completion snapshot fail/error count 2, strict-matrix fail, and `drift_error_count=6`; it explicitly does not approve metrics, pages, owner signoff, governance records, secret hygiene, direct App MCP/GitNexus evidence, or route certification.

...
- `docs/audits/2026-06-10-calculation-p1-owner-decision-matrix.md` converts the 10 remaining open calculation/display P1 findings into owner-decision rows and records P1-08 bond-dashboard null handling as verified closed.
...
- `docs/audits/2026-06-10-owner-approval-fail-closed-snapshot.json` records the current machine-readable owner gate: 7 pending pages, 0 captured approvals, 0 closure-approved pages, 84 action items, strict gate rejected 7/7, and owner approval tests at 90 passed.
- `docs/audits/2026-06-10-direct-app-mcp-gitnexus-tool-surface-snapshot.json` records the direct Codex App MCP/GitNexus tool-surface gap: current tool discovery found 0 direct MOSS/GitNexus tools even though local registration files declare the expected servers.
...
- `docs/audits/2026-06-10-real-backend-smoke-runbook.md` defines the exact live-backend browser-smoke command, exclusions, acceptance criteria, and result-capture fields for the still-open full real-backend smoke gate.
- `tests/test_system_audit_manifest_contract.py` is the narrow audit-package consistency guard for artifact links, fail-closed flags, owner non-approval boundaries, direct App MCP/GitNexus gap boundaries, and manifest count alignment.
...
- Added a machine-readable fail-closed owner snapshot: 7 pending pages, 0 captured approvals, 0 closure-approved pages, 84 action items, strict `--require-captured` rejection for all seven pages, and owner approval status tests at 90 passed.
...
The companion calculation/display-logic audit found no P0 issue. It originally recorded 11 P1 findings; P1-08 bond-dashboard null handling and false MoM has since been verified closed in the current worktree, leaving 10 open P1 findings. Several remaining items are not simple implementation bugs: the repository contains two contradictory implementations or sign/unit conventions, and both sides have tests that preserve their current behavior.
...
Owner review aid:
- `docs/audits/2026-06-10-calculation-p1-owner-decision-matrix.md` lists the 10 remaining P1 decisions, candidate conventions, proposed review defaults, impact if unresolved, and closure evidence; it separately records P1-08 as verified closed.
...
- After narrowing `.gitleaks.toml` for already-ignored generated/local directories and the governed Risk Tensor metric-key false positive, `python scripts\supply_chain_security_scan.py --tool gitleaks --report-dir test_output\security-scans` -> exit 1 with 2 findings, both in ignored/untracked `config/.env` (`MOSS_TUSHARE_TOKEN`, `STITCH_API_KEY`, values redacted).
- `git check-ignore -v config/.env` -> ignored by `.gitignore`; `git ls-files -- config/.env` -> not tracked.
- `python scripts\supply_chain_security_scan.py --tool osv --report-dir test_output\security-scans` -> exit 0; parsed `osv-report.json` shows 0 results and 0 vulnerabilities.
...
- `python scripts\codex_page_readiness.py --all` -> 39 pages, 39 static-pass, 0 blocked, 27 run-supported, 7 business-owner approval pending, 84 approval action items.
...
- `pytest tests/test_system_audit_manifest_contract.py -q` -> 8 passed. This checks that the manifest references existing artifacts, stays fail-closed, matches coverage/fresh-verification counts, preserves owner-approval, direct App MCP/GitNexus gap, Ledger PnL direct-record, local secret hygiene, and owner-review non-approval boundaries, and keeps the real-backend smoke runbook non-certifying.
- `tool_search query: moss metric contracts lineage evidence data catalog gitnexus MCP tools` -> 0 tools found in the current Codex App tool surface at `2026-06-10T13:05:58+08:00`; local `.codex/config.toml` and `.mcp.json` still declare the expected servers, so the direct App MCP/GitNexus evidence gap remains open.
- `python scripts\emit_ledger_pnl_governance_record.py`; direct `cache_manifest.jsonl` search; `python scripts\codex_page_readiness.py --page-slug ledger-pnl` refreshed at `2026-06-10T12:47:03+08:00` -> dry-run candidate has `validation_status=ready_for_audit_review`, `record_write_status=not_requested`, `existing_record_line=null`, no written direct record search matches, and page readiness remains `formal_use_allowed=false` / `closure_approved=false`.
- `python scripts\supply_chain_security_scan.py --dry-run`; `git check-ignore -v config/.env`; `git ls-files -- config/.env`; `git status --ignored --short -- config/.env` refreshed at `2026-06-10T12:57:33+08:00` -> gitleaks plan includes `--redact`; `config/.env` exists, is ignored by `.gitignore:4`, has no tracked path, and is reported as ignored. Secret values were not read or captured.
...
- Fresh Ledger PnL direct-record drill-down: `python scripts\emit_ledger_pnl_governance_record.py` -> dry-run only, `existing_record_line=null`, `record_write_status=not_requested`, and `validation_status=ready_for_audit_review` for a field-complete `PAGE-LEDGER-PNL-001` candidate keyed by `ledger_pnl.summary:2026-05-31:ALL`.
...
- `python scripts\codex_page_readiness.py --page-slug ledger-pnl` -> static-pass, `formal_use_allowed=false`, `business_owner_approval_captured=false`, `governance_record_validation=null`, and required direct-record/page-review gaps preserved.
- `python -m pytest tests\test_project_mcp_servers.py::test_lineage_evidence_governance_record_blueprint_queue_covers_default_open_gaps tests\test_ledger_pnl_business_owner_approval_status.py -q --tb=short --basetemp=.codex-tmp\pytest-basetemp\ledger-pnl-direct-record-audit` -> 13 passed.
...
- `python -m pytest tests/test_product_category_pnl_business_owner_approval_status.py tests/test_balance_analysis_business_owner_approval_status.py tests/test_average_balance_business_owner_approval_status.py tests/test_ledger_pnl_business_owner_approval_status.py tests/test_pnl_attribution_business_owner_approval_status.py tests/test_bond_analysis_business_owner_approval_status.py tests/test_stock_analysis_business_owner_approval_status.py -q` -> 90 passed.
...
- `docs/audits/2026-06-10-system-audit-manifest.json` records fresh verification at `2026-06-10T13:05:58+08:00` for page readiness, route scope, and business display coverage. The manifest explicitly keeps `approves_metrics=false`, `approves_pages=false`, `captures_business_owner_approval=false`, `writes_governance_records=false`, and `certifies_routes=false`.
- `docs/audits/2026-06-10-owner-approval-fail-closed-snapshot.json` records the seven-page owner gate as fail-closed, refreshed at `2026-06-10T13:01:48+08:00`: 7 pending, 0 captured, 0 closure-approved, 84 action items, strict approval capture rejected 7/7, and 90 owner approval status tests passed.
- `docs/audits/2026-06-10-direct-app-mcp-gitnexus-tool-surface-snapshot.json` records the direct App MCP/GitNexus evidence gap as fail-closed: 0 direct tools discovered in the current App surface, local config declarations present, local stdio evidence marked fallback-only, and no approval/closure effect.
- `docs/audits/2026-06-10-ledger-pnl-direct-governance-record-snapshot.json` records the Ledger PnL direct-record gap as fail-closed, refreshed at `2026-06-10T12:47:03+08:00`: dry-run only, no written direct record match, static-pass readiness only, `formal_use_allowed=false`, and `closure_approved=false`.
- `docs/audits/2026-06-10-local-secret-hygiene-snapshot.json` records the local secret hygiene gap as fail-closed, refreshed at `2026-06-10T12:57:33+08:00`: secret names only, values not captured, `config/.env` ignored and untracked, and clean-runner/owner action still required.

## Residual Risks

- MCP App-surface gap: direct Codex App tools for MOSS MCP and GitNexus were not exposed in this session. Fresh tool discovery at 2026-06-10T13:05:58+08:00 for `moss metric contracts lineage evidence data catalog gitnexus MCP tools` returned 0 tools. Local `.codex/config.toml` and `.mcp.json` declare the expected servers, and local MOSS MCP stdio launchers, tools/resources, read-only queue calls, and the full project MCP server contract suite pass. The remaining gap is client/session direct-tool exposure plus page-level manual review, not local MOSS MCP server registration.
...
- Calculation/display-logic audit still has 10 open P1 items. Several need business owner rulings before code changes because existing tests currently freeze contradictory conventions. P1-08 bond-dashboard null handling is closed by the targeted formatter/page test slice.
...
- Browser smoke is now split by data-source contract: full mock smoke passes as the stable first-screen/a11y gate, and the monthly operating analysis audit real-client smoke passes separately. A true full real-backend smoke remains open because several routes still need a live backend/MCP authority target to make their real-mode ready selectors and controls meaningful; `docs/audits/2026-06-10-real-backend-smoke-runbook.md` now defines the closure run without counting mock, route-mocked, or dry-run evidence.
- External secret scan still fails in this local workspace because ignored/untracked `config/.env` contains two detected local credential values. The file is not tracked, but the values should be treated as sensitive local secrets and rotated/confirmed according to owner policy if exposure is suspected.
...
1. Use `docs/audits/2026-06-10-direct-app-mcp-gitnexus-tool-surface-runbook.md` to retry direct App MCP/GitNexus discovery in a fresh session, then compare any direct App evidence against the local stdio evidence already recorded.
2. Attach page-specific local MCP evidence packets for the 7 business-owner-approval-pending pages, then repeat through direct Codex App MCP tools when a fresh session exposes them.
3. Route the 10 remaining calculation/display P1 items to business-owner rule decisions, then update `docs/calc_rules.md`, the authoritative implementation layer, and the tests that currently freeze the losing convention.
4. Use `docs/audits/2026-06-10-owner-review-brief.zh.md` for the owner meeting and record meeting outputs in `docs/audits/2026-06-10-owner-decision-capture-template.zh.md` before copying approved inputs into authoritative templates and rules.
5. Work through the fresh owner-approval action matrix: fill owner identity/decision/signature fields, complete the page-specific evidence reviews, and keep `ledger-pnl` last until the approved governance workflow writes or locates the direct page/API record and the follow-up governance validation passes.
6. Use `docs/audits/2026-06-10-ledger-pnl-direct-governance-record-runbook.md` for Ledger PnL only after governance-owner authorization; dry-run evidence remains non-certifying.
7. Use `docs/audits/2026-06-10-local-secret-hygiene-runbook.md` to rotate, remove, or owner-accept the local `config/.env` findings without copying values into the repo.
8. Re-run `pytest tests/test_system_audit_manifest_contract.py -q` after edits to the manifest, coverage report, owner brief, capture template, action register, direct App MCP/GitNexus artifacts, Ledger PnL direct-record artifacts, or local secret hygiene artifacts.
9. Re-run the full browser smoke suite against a real backend target using `docs/audits/2026-06-10-real-backend-smoke-runbook.md` after MCP authority evidence is exposed; the current fresh browser evidence covers the full mock first-screen/a11y gate plus the monthly operating analysis real-client route-mocked gate.
10. Re-run gitleaks in a clean CI runner and resolve or owner-accept the local `config/.env` credential findings without committing secret values.

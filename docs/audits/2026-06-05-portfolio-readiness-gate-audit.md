...
Portfolio full-closure evidence checks passed on the current worktree:
...
- `bond_maturity_date_remediation_required`: source bond facts still lack maturity dates for material balances, so duration and liquidity-gap evidence remain incomplete;
...
- GitNexus evidence was unavailable through this Codex App pass and should be rerun when the MCP surface is exposed; it does not replace or add to the current eight score blockers.

...
2. **P2 - Ledger PnL summary API metadata conflicts with candidate metric governance.** `MTR-LPN-001` through `MTR-LPN-003` are `candidate` / `pending_confirmation=true` with no bound summary golden sample, but `/api/ledger-pnl/summary` and `/api/ledger-pnl/data` currently emit `result_meta.basis=formal` and `formal_use_allowed=true`.
...
- Ledger PnL page-contract bundle check:
  ...
  - Current residual evidence note: `PAGE-LEDGER-PNL-001` has no dedicated capture-ready summary golden sample; `GS-LEDGER-PNL-FIN-IND-202603-B` is a source-contract fixture and must not promote formal-pending or candidate QDB values into formal financial indicator truth. Follow-up audit evidence also found a metadata mismatch: the metric dictionary keeps `MTR-LPN-001` through `MTR-LPN-003` as candidate / `pending_confirmation=true`, but the live summary and detail envelopes still report `result_meta.basis=formal` and `formal_use_allowed=true`.
  ...
  - 2026-06-03 runtime evidence: direct service calls for `ledger_pnl_summary_envelope(..., report_date="2026-03-31")` and `ledger_pnl_data_envelope(..., report_date="2026-03-31")` returned `basis=formal`, `formal_use_allowed=true`, empty `tables_used`, and no requested/resolved/as_of date metadata, while `ledger_pnl_formal_financial_indicator_contract_envelope(report_month="202603")` correctly returned `basis=ledger`, `formal_use_allowed=false`, `date_basis=report_month_end`, and `evidence_rows=21`.
  - Fresh focused checks for this follow-up: `python -m pytest tests/test_ledger_pnl_service.py tests/test_ledger_pnl_formal_financial_indicator_golden_sample.py -q` passed with `11 passed`; `npm run test -- src/test/LedgerPnlPage.test.tsx` passed with `1` file and `12` tests. These checks verify existing behavior and source-contract display, not closure of the summary metadata mismatch.
...
2. Use the 26/26 page trace bundles as the routing map for the next audit pass: data-catalog/date-lineage review by page, starting with the highest-risk governed metric pages and known mixed-source surfaces.
3. Decide whether the broad backend Ruff backlog is a release blocker or a separately tracked cleanup stream.
4. Use direct MOSS MCP contract/lineage/catalog tools when exposed, or invoke the equivalent project scripts, to audit source lineage and date semantics for ledger-pnl, stock-analysis, agent workbench, commodity ingest, and governed metric pages.

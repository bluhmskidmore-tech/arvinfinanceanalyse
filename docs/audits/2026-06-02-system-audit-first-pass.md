...
2. **P1 - Formal calculation boundary drift remains in `bond_analytics_service`.** DV01 risk/reconciliation/movement/action-plan pure calculations, action-attribution bond-line shaping, and action-attribution success payload rules now live in `core_finance`, but action-attribution orchestration still mixes PnL lookup, placeholder handling, schema validation, and result-envelope assembly in the service layer.
...
5. **P2 - Page-level MCP trace bundle coverage now has first-pass closure.** Local MCP fallback evidence now directly reads the MOSS metric-contract, data-catalog, data-quality, and lineage providers. The seeded bundle registry now resolves 27 unique page IDs after adding `PAGE-PNL-BY-BUSINESS-001` as a dedicated Business Type PnL surface, including the remaining Agent, Cube Query, and module-home surfaces. This closes the bundle-coverage gap, but it does not replace full data-catalog/date-lineage review for every governed metric page.
...
- **P1 regression guard - Backend route authorization inventory now fails on unclassified surfaces and missing policy scopes.** Route inventory tests require backend GET/read-like POST handlers to reach authorization unless explicitly public/echo, mutation handlers to be authorized unless explicitly reserved, and authorized surfaces to expose stable resource/action policy scopes.
- **P1 partially remediated - Bond Analytics calculations moved behind core_finance.** DV01 shock parsing/expansion, face-weighted duration, absolute-exposure shares, tenor/bond/issuer payloads, reconciliation payloads, movement attribution payloads, action-plan scenario/tenor/issuer/bond recommendation payloads, action-attribution bond-line shaping, and action-attribution success payload rules now live under `backend/app/core_finance/`.
...
- `docs/audits/2026-06-02-system-audit-first-pass.md`
  ...
  - Updated the registry note again after adding the dedicated Business Type PnL bundle; `product_page_trace_bundles()` now resolves 27 unique seeded page IDs, including `PAGE-PNL-BY-BUSINESS-001`.
...
- `backend/app/core_finance/action_attribution.py` now owns the bond analytics row -> action-attribution core input line mapping via `bond_analytics_action_line_payload(...)`, and action-attribution success payload rules via `build_action_attribution_success_payload(...)`, including ready status, available/missing/blocked component lists, computed-at propagation, warning de-duplication, and warning detail rows.
- `backend/app/services/bond_analytics_service.py` now calls `dv01_core` for `get_dv01_risk`, `get_dv01_reconciliation`, `get_dv01_movement`, shared DV01 movement summaries, and `get_dv01_action_plan` recommendation payloads, and calls `bond_analytics_action_line_payload(...)`, `compute_action_attribution_bonds(...)`, and `build_action_attribution_success_payload(...)` for the action-attribution success path, while keeping repository access, limit-config resolution, PnL lookup, schema validation, sorting, placeholder handling, and result metadata/envelope assembly in the service layer.
...
- Remaining drift: `backend/app/services/bond_analytics_service.py` still defines `_build_action_attribution_pnl_by_key`, `_build_action_attribution_success_response`, and `get_action_attribution`, while `get_action_attribution` still orchestrates PnL lookup, placeholder response handling, schema validation, formal metadata, and result-envelope assembly.
...
Recommendation: continue with a bounded follow-up for the remaining action-attribution orchestration, or document a temporary exception that keeps PnL/repository lookup, placeholder response handling, schema validation, and result-envelope assembly in the service layer. Until then, treat the residual as an architectural watch item rather than fully closed.
...
- `product_page_trace_bundles()` now resolves 27 unique seeded page IDs after adding `PAGE-PNL-BY-BUSINESS-001`; the Product Category bundle still keeps the older `PAGE-PROD-CAT-001` as its canonical `page_id` while accepting `PAGE-PROD-CAT-PNL-001` as an alias:
...
Impact: the bundle workflow can now return first-pass trace bundles for the seeded page surfaces, including the newly separated Business Type PnL page. This is not a full system-wide release claim because most page-level data-catalog/date reviews and broader browser/a11y coverage remain incomplete.
...
- Local MCP fallback evidence:
  - `moss-metric-contracts`: resources and tools are callable through the repository's JSON-RPC MCP process; contract docs exist; seeded trace bundles now resolve 27 unique page IDs, including `PAGE-PNL-BY-BUSINESS-001`, `agent`, `cube-query`, `portfolio-home`, `market-home`, `risk-home`, `performance-home`, and `reports-home`.
...
- Page-level MCP bundle coverage now resolves 27 unique seeded page IDs in `product_page_trace_bundles()`, including `PAGE-PNL-BY-BUSINESS-001`. This coverage is first-pass traceability only; it does not prove every page's data lineage, date semantics, browser state, or golden-sample coverage.
...
2. Continue the bounded architecture remediation for `bond_analytics_service` calculation drift: move any remaining action-attribution pure response payload shaping behind `core_finance`, or document a temporary exception with tests while leaving PnL/repository lookup, placeholder handling, and result-envelope assembly in the service layer.
3. Audit the next highest-risk governed metric pages for data-catalog/date-lineage gaps using the 27 seeded page trace bundles as the routing map, starting with pages that mix formal source tables with candidate or tooling display fields.

...
- `python -m pytest tests/test_project_mcp_servers.py -q`: fresh continuation run passed with `15 passed`.
- Local MCP fallback evidence:
  - `moss-metric-contracts`: resources and tools are callable through the repository's JSON-RPC MCP process; contract docs exist; seeded trace bundles are available for `product-category-pnl`, `dashboard-home`, and `risk-tensor`.

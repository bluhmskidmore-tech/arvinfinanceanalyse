...
| System audit pulse | `scripts/system_audit_pulse.py`; `tests/test_system_audit_pulse.py` | read-only quick monitor combines route-scope, business display coverage, completion snapshot, and last full readiness evidence; current pulse is pass but `completion_state=not_complete` with 5 open blockers |
...
| Audit package contract tests | `tests/test_system_audit_manifest_contract.py`; `tests/test_system_audit_completion_snapshot_verifier.py`; `scripts/verify_system_audit_completion_snapshot.py` | checks artifact links, fail-closed flags, direct App MCP/GitNexus gap boundaries, completion snapshot alignment, owner/governance packet and Chinese brief coverage, calculation P1 engineering-prework map coverage, and coverage/fresh-verification counts |
...
| `scripts/system_audit_pulse.py` | Fast read-only monitoring for count drift across route-scope, business display coverage, completion snapshot, and last full readiness evidence. | Approving metrics/pages, writing governance records, clearing secrets, direct App MCP/GitNexus closure, or replacing the slower all-page readiness scan when full catalog/date evidence is required. |
...
6. For a quick live monitor, run `python scripts\system_audit_pulse.py`; use `--include-full-readiness` only when the slower all-page catalog/date evidence needs to be refreshed.
7. After audit package edits, run `pytest tests/test_system_audit_manifest_contract.py tests/test_system_audit_completion_snapshot_verifier.py tests/test_system_audit_pulse.py -q`, then `python scripts\verify_system_audit_completion_snapshot.py`.
...
12. For calculation decisions, review `2026-06-10-calculation-p1-owner-decision-matrix.md` with the business owner.
...
16. For the Ledger PnL direct-record gap, read `2026-06-10-ledger-pnl-direct-governance-record-snapshot.json`, then use `2026-06-10-ledger-pnl-direct-governance-record-runbook.md` only under approved governance-owner authorization.

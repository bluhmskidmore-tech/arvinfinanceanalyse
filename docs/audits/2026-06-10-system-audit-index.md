...
| Calculation/display decisions | `2026-06-10-calculation-p1-owner-decision-matrix.md`; `2026-06-10-calculation-p1-owner-decision-snapshot.json` | 10 remaining P1 owner-decision rows; snapshot is fail-closed with 10 pending template rows and P1-08 bond-dashboard null handling verified closed |
...
| Local secret hygiene | `2026-06-10-local-secret-hygiene-snapshot.json`; `2026-06-10-local-secret-hygiene-runbook.md` | last full refresh at 2026-06-10T15:45:10+08:00: OSV 0 vulnerabilities; latest non-closing retry at 2026-06-10T17:30:56+08:00: redacted gitleaks still has 2 ignored/untracked `config/.env` findings by secret name only, OSV retry was blocked by proxy refusal, values are not captured; latest boundary-only recheck at 2026-06-10T19:33:20+08:00 still shows ignored/untracked without reading values |
| Business display coverage | `business-display-coverage-report.json` | 26 tracked routes, 0 route gaps |
| System audit pulse | `2026-06-10-system-audit-pulse-snapshot.json`; `scripts/system_audit_pulse.py`; `tests/test_system_audit_pulse.py` | read-only quick monitor combines route-scope, business display coverage, completion snapshot, and last full readiness evidence; current snapshot is pass but `completion_state=not_complete` with 5 open blockers |
...
| `2026-06-10-calculation-p1-owner-decision-snapshot.json` | Machine-readable read-only check that the matrix, capture template, and engineering prework map still expose 10 open P1 decisions and no captured owner choices. | Choosing conventions, changing implementation, or approving metrics/pages. |
...
| `2026-06-10-ledger-pnl-direct-governance-record-snapshot.json` | Machine-readable Ledger PnL direct-record gap status, dry-run candidate key, written-record search result, refresh command, and readiness boundary. | Writing the governance record, proving live execution, or approving Ledger PnL. |
...
| `2026-06-10-system-audit-pulse-snapshot.json` | Machine-readable point-in-time pulse for route-scope, business display coverage, completion snapshot, last full readiness evidence, and open blocker counts. | Approving metrics/pages, writing governance records, clearing secrets, direct App MCP/GitNexus closure, replacing owner approvals, or treating `status=pass` as audit completion. |
| `scripts/system_audit_pulse.py` | Fast read-only monitoring for count drift across route-scope, business display coverage, completion snapshot, and last full readiness evidence; use `--format markdown` for the human-readable watch board. | Approving metrics/pages, writing governance records, clearing secrets, direct App MCP/GitNexus closure, or replacing the slower all-page readiness scan when full catalog/date evidence is required. |
| `2026-06-10-system-audit-monitoring-snapshot.json` | Unified read-only monitoring bundle for calculation decisions, direct App MCP/GitNexus tool surface, local secret hygiene, Ledger PnL direct governance record, completion verifier, and pulse status. | Granting page closure, owner approval, formal use, secret clearance, direct App evidence, or governance-record writes. |
| `scripts/verify_system_audit_monitoring_snapshot.py` | Verifying the monitoring snapshot stays aligned with the manifest, pulse, completion snapshot, and blocker-specific snapshots. | Refreshing evidence, writing outputs, approving blockers, or replacing the blocker-specific closure gates. |
...
6. For a quick live monitor, run `python scripts\system_audit_pulse.py`; add `--format markdown` for a readable watch board, and use `--include-full-readiness` only when the slower all-page catalog/date evidence needs to be refreshed.
7. After audit package edits, run `pytest tests/test_system_audit_manifest_contract.py tests/test_system_audit_completion_snapshot_verifier.py tests/test_system_audit_pulse.py tests/test_system_audit_monitoring_snapshot_verifier.py -q`, then `python scripts\verify_system_audit_completion_snapshot.py` and `python scripts\verify_system_audit_monitoring_snapshot.py`.
...
12. For calculation decisions, refresh `2026-06-10-calculation-p1-owner-decision-snapshot.json` with `python scripts\refresh_calculation_p1_owner_decision_snapshot.py`, then review `2026-06-10-calculation-p1-owner-decision-matrix.md` with the business owner.
...
15. For the direct App MCP/GitNexus gap, read `2026-06-10-direct-app-mcp-gitnexus-tool-surface-snapshot.json`, then use `2026-06-10-direct-app-mcp-gitnexus-tool-surface-runbook.md` in a fresh session.
16. For the Ledger PnL direct-record gap, refresh/read `2026-06-10-ledger-pnl-direct-governance-record-snapshot.json` with `python scripts\refresh_ledger_pnl_direct_governance_record_snapshot.py`, then use `2026-06-10-ledger-pnl-direct-governance-record-runbook.md` only under approved governance-owner authorization.
17. For local secret hygiene, read `2026-06-10-local-secret-hygiene-snapshot.json`, then use `2026-06-10-local-secret-hygiene-runbook.md` without exposing values.

...
- The response contract is `candidate-financial-indicator-period-comparison-v3`. In addition to the fixed seven rows it returns one backend-owned `net_interest_component_bridge` and one backend-owned `net_interest_account_driver_summary`; clients that still require v1 or v2 must fail closed rather than silently dropping either explanation layer.
...
- The eager `net_interest_account_driver_summary` reuses the same three already-loaded ledger periods, frozen rules, source locks, and four exact component-detail calculations. Its grain is explicitly `component_account`: an account that participates in more than one governed component is not silently merged across rule contexts. The parent response exposes only ranked summary rows; complete matched terms, three-period balances, and source locators remain in the existing lazy component-detail endpoint.
- The backend partitions every `contributing` component-account row into positive lift, negative drag, or exact-zero neutral counts, returns the unrounded lift/drag/account totals, and ranks at most three lifts and three drags by unrounded Decimal contribution. Lifts sort descending and drags ascending, with fixed component order and account code as deterministic ties. The frontend preserves this order and formats the Decimal strings; it does not add, regroup, rerank, or derive percentages.
- The account contribution total must equal the bridge/net-interest delta exactly before `status=available` and `foot_status=passed` may expose Top rows. `excluded_offset` rows are counted separately and never enter amounts or ranking. If the bridge or any component detail is not evaluable, the summary exposes no amounts, counts, or rows. A non-zero cross-account foot exposes only net total, account total, and reconciliation diagnostics; lift/drag totals, counts, and ranked rows remain hidden. The summary does not return concentration, materiality, volume, rate, spread, or business-cause claims.
...
- The comparison idempotency key binds the contract version, exact three months, complete source/hash evidence, rule hash, full-scope replay evidence, the complete seven-row response, the complete bridge evidence, and the complete account-driver summary including totals, counts, ranked rows, and foot status. `metric_status=candidate`, `formal_use_allowed=false`, and `certification_effect=none` are invariant.
- React consumes backend `current_value_yi`, `previous_value_yi`, `delta_yi`, `change_rate`, component changes, signed contributions, account-driver totals/counts/ranks, and foot evidence without recomputation. It may format Decimal strings and enforce response-shape/cross-field identity only. The operating-analysis view leads with the comparable/not-comparable count and source warning, then shows the account-driver summary before the complete-scope gap, compact bridge, and seven-row table. It distinguishes natural-month from month-end comparison, shows unavailable values as `--`, preserves the existing lazy component drill-through, and cancels in-flight reads when the view is unmounted. Loading, transport failure, wrong-month/malformed contract, no-month, account-summary-unavailable/failed, bridge-unavailable/failed, partial, and fully unavailable states are explicit.

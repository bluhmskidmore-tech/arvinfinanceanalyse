...
- `/api/pnl/by-business-analysis?dimension=currency` uses the confirmed original-asset-currency rule: instrument codes beginning with `J1` are grouped as `USD`; `JM`, `J4`, and all other codes are grouped as `CNY`. This dimension changes grouping only; PnL, ADB, current balance, FTP cost, and FTP-after-PnL remain CNY-equivalent amounts.

...
| `GS-PNL-BUSINESS-INSIGHTS-A` | `PAGE-CONTRACT-PENDING:/pnl-by-business-insights` / `GET /api/pnl/by-business-candidate-insights` | `MTR-PNLBIZ-001`, `MTR-PNLBIZ-002`, `MTR-PNLBIZ-003`, `MTR-PNLBIZ-004`, `MTR-PNLBIZ-005`, `MTR-PNLBIZ-006` | Candidate business-type insights DTO (HHI concentration, top-N share, negative-FTP persistence, share drift), analytical `result_meta`, and `formal_use_allowed=false` are sample truth; `MTR-PNLBIZ-001`~`005` are a secondary re-aggregation of `pnl_by_business_ytd_envelope` / `pnl_by_business_monthly_envelope`; `MTR-PNLBIZ-006` (`reconciliation_diagnostics`) is a separate formal 对账健康度诊断趋势 batched from the existing single-day untraced-row SQL and is not a business-analysis re-aggregation; none of the six approve `PAGE-PNL-BY-BUSINESS-001` (`docs/page_contracts.md` §14.8.1), formal PnL truth, a concentration limit, an FTP rate caliber, governance closure, manual audit, or owner approval | `tests/test_pnl_by_business_candidate_insights_contract.py`; `tests/test_golden_samples_capture_ready.py` |
...
| `pnl-by-business-insights` | 新增 6 条 `candidate`：`MTR-PNLBIZ-001`~`MTR-PNLBIZ-006` | `PAGE-CONTRACT-PENDING:/pnl-by-business-insights` | `GS-PNL-BUSINESS-INSIGHTS-A` | dedicated candidate DTO 已 capture-ready 但未审批；`MTR-PNLBIZ-001`~`005`（业务种类集中度 HHI/前三大占比、负 FTP 持续性、份额漂移）均为 `pnl_by_business_ytd_envelope`/`pnl_by_business_monthly_envelope` 已有父级行结果的二次聚合，不新查任何原始表；`MTR-PNLBIZ-006`（对账健康度诊断趋势）是独立顶层字段 `reconciliation_diagnostics`，直接批量复用既有单日 `fact_formal_pnl_fi` 未追溯诊断 SQL，属于对账/数据质量诊断而非业务分析二次聚合，字段一律使用 `untraced_` 前缀，不与前 5 条业务分析字段混排；六条均不替代或扩展 `PAGE-PNL-BY-BUSINESS-001`（`docs/page_contracts.md` §14.8.1，本轮无新批准 `MTR-*` 绑定） |
...
- `MTR-PNLBIZ-006` 未追溯FI占比（近12个月末趋势）: `status=candidate`; `display_unit=%`; `precision=2`; `sign_rule=unsigned percent`; `null_rule=null -> --`; `source_endpoint=GET /api/pnl/by-business-candidate-insights`; `owner=TBD`; `last_reviewed=2026-07-07`; `bound_page_id=PAGE-CONTRACT-PENDING:/pnl-by-business-insights`; `bound_sample_id=GS-PNL-BUSINESS-INSIGHTS-A`; `pending_confirmation=true`. 备注：formal对账健康度诊断趋势，来源于既有单日未追溯诊断的批量化，不构成业务贡献/拖累结论，不得与月报/YTD业务结论混用，边界与 docs/page_contracts.md §14.8.1 G节现有未追溯说明一致。

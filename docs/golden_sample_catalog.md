...
| `GS-PNL-BUSINESS-INSIGHTS-A` | `GET /api/pnl/by-business-insights` | `approved` | `2026-02-28` | `tests/test_golden_samples_capture_ready.py`; `tests/test_pnl_by_business_insights_contract.py` | approved derived business-analysis formulas and DTO sample; not underlying source-PnL truth, a concentration limit, or an FTP-rate approval; `MTR-PNLBIZ-006` remains diagnostic-only |

...
| YTM / duration / convexity compounding | Current formulas treat `ytm` as nominal annual yield and divide by coupon frequency from `interest_mode`: annual=1, semi-annual=2, quarterly=4, unknown/bullet=1. Convexity is the standard cash-flow second derivative and shares the discounted-cash-flow traversal with Macaulay duration; zero-coupon, bullet, or non-positive-YTM cases use the corresponding single-cash-flow closed form. | Recommend `yield_compounding=nominal_annual_with_coupon_frequency`; scope to fixed-rate vanilla bonds, with floating/callable/amortizing/defaulted bonds excluded or downgraded. | `recommended_pending_owner` |
...
- Day-count / compounding evidence:
  ...
  - `backend/app/core_finance/bond_analytics/common.py`: Macaulay duration and standard cash-flow convexity share one discounted-cash-flow traversal using `coupon_rate / freq` and `ytm / freq`; modified duration uses `1 + ytm / freq`; single-cash-flow cases use the exact closed-form convexity `t(t + 1/f) / (1 + y/f)^2`.

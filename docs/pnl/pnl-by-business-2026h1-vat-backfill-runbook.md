# `/pnl-by-business` 2026 年上半年 FI 514/517 与 J1 汇率回溯操作手册

## 适用口径

- 期间：2026-01-01 至 2026-06-30。
- 正式 FI：仅现有 V1 规则明确认定的应税品种，其 514 源金额按含税额处理并除以 `1.06`；H/A/T 仅表示投资分类，不作为增值税应税判断，免税品种不处理。
- 正式 FI 517：源文件为累计已实现口径，按会计导出符号反向并除以 `1.06`；H/A 使用 `realized_formal`，T 使用 `realized_incremental`，避免与 516 重叠。
- 非标 JM：同一期间内按含税额除以 `1.06`；其他非标代码不处理。
- 外币委外 J1：514/516/517 按各月正式 USD/CNY 月末汇率折人民币；JM 与 J4 不应用该换汇规则。
- 2025 年及 2026-07-01 之后不应用本次回溯规则。
- 正式事实规则版本必须为 `rv_pnl_phase2_materialize_v3`。页面和预计算在发现旧版本事实时会拒绝继续读取。
...
- 1—6 月正式事实的 `rule_version` 均为 `rv_pnl_phase2_materialize_v3`。

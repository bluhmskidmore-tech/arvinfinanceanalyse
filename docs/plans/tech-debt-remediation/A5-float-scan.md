...
| `core_finance/bond_duration.py:61` | `max(0.0, (maturity - report).days / 365.0)` | 期限代理年限（时间量纲）；207 行 `Decimal(str(_estimate_duration_proxy_years(...)))` 立即量化，Macaulay/修正久期/凸性全 Decimal（67-268 行，`common.py` 同） |

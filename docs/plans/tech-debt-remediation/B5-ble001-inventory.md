...
| 批次 2 清偿后全库实测余额（2026-08-12，同命令） | **127** |

## 按文件计数（降序）

| 数量 | 文件 |
| ---: | --- |
| 0（原 11，批次 2 已清偿：11 处窄化为 KpiWorkbenchError） | backend/app/api/routes/kpi.py |
...
| 0（原 4，批次 2 已清偿：4 处 noqa，API 边界治理/broker 兜底） | backend/app/api/routes/ledger.py |
...
| 0（原 4，批次 2 已清偿：4 处 noqa，失败恢复/清理路径） | backend/app/tasks/materialize.py |
...
| 0（原 1，批次 2 已清偿：1 处 noqa，BaseException 耐久性写入边界） | backend/app/tasks/ledger_classification_backfill.py |
...
合计（冻结基线）：179 处，72 个文件。批次 1 清偿 26 处（3 个文件清零）后，全库实测余额 147 处（2026-08-12，差额含其他并行批次同步清偿与代码演进）。批次 2 清偿 20 处（4 个文件清零）后，全库实测余额 127 处（2026-08-12）。
...
### 批次 2（2026-08-12）— 20 处，余额 147 → 127（实测）

| 文件 | 原计数 | 处置 |
| --- | ---: | --- |
| backend/app/api/routes/kpi.py | 11 | 11 处窄化为 `kpi_workbench_service.KpiWorkbenchError`（服务层对外只抛 KpiNotFound/KpiInvalidDate/KpiStorage 三个子类，`_raise_workbench_http_error` 映射 404/422/503；未知异常原本也会被 helper 重抛为 500，窄化后直接透传，行为等价、无吞错） |
| backend/app/api/routes/ledger.py | 4 | 4 处 `noqa: BLE001`（API 边界兜底：治理后端 jsonl/sql 写入与 broker 入队 vendor SDK 异常面无界，失败迁移落账属清理路径；每处已有 `logger.error` 含 error_type 并返回结构化 503） |
| backend/app/tasks/materialize.py | 4 | 4 处 `noqa: BLE001`（构建失败顶层兜底：落败迹、恢复快照后原样重抛；证据落账异常暂存后作为 RuntimeError 重抛；两处备份清理路径已 `logger.warning`，均无吞错） |
| backend/app/tasks/ledger_classification_backfill.py | 1 | 1 处 `noqa: BLE001`（`BaseException` 耐久性写入边界：须捕获含 KeyboardInterrupt 的一切失败以写入败迹回执，随后原样重抛） |

验证：四个文件 `ruff --select BLE001` 清零、全量 ruff 通过；`py -m py_compile` 通过；配套测试 `test_kpi_api.py`、`test_ledger_import_flow.py`、`test_ledger_analytics_api.py`、`test_materialize_flow.py`、`test_ledger_classification_backfill.py` 共 142 passed / 3 skipped（既有环境条件跳过：银行台账样本文件与硬链接不可用）。全库复扫余额 127（`py -m ruff check backend --select BLE001 --statistics`）。

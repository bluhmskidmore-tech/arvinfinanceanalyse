...
- Never invoke a bare `python`: on developer machines it is frequently shadowed by an unrelated venv.
  Use the repo interpreter — Windows `.\.venv\Scripts\python.exe`, POSIX `.venv/bin/python`. In CI the
  `uv sync --frozen` venv is prepended to `PATH`, which is why the workflow files can use plain `python`.
- From the repository root, run targeted pytest as `.\.venv\Scripts\python.exe -m pytest tests/<target>.py -q`.
...
- For the default MCP feedback loop, run `.\.venv\Scripts\python.exe -m pytest -q -m mcp_fast tests/test_project_mcp_fast_contracts.py`; it uses isolated governance and DuckDB paths.
- Run `.\.venv\Scripts\python.exe scripts\backend_release_suite.py --mcp-profile full` only when shared MCP behavior changes or when performing the scheduled/full release check.

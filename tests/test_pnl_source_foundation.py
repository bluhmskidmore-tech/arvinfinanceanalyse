...
def test_pnl_refresh_selects_copy_suffixed_nonstd_range_for_june(tmp_path) -> None:
    ...
    data_root = tmp_path / "data_input" / "pnl_514"
    data_root.mkdir(parents=True, exist_ok=True)
    workbook = Workbook()
    ...
    sheet.append(["2026-05-31", "利息", "5010", "FIOA", "JM0001", "贷", "51401000004", "106"])
    sheet.append(["2026-06-30", "利息", "5010", "FIOA", "JM0001", "贷", "51401000004", "212"])
    workbook.save(data_root / "非标514-20260101-0630(1).xlsx")
    ...
    assert refresh.report_date == "2026-06-30"
    assert [row["voucher_date"] for row in refresh.nonstd_rows_by_type["514"]] == ["2026-06-30"]
    assert refresh.nonstd_rows_by_type["514"][0]["raw_amount"] == source_module.Decimal("212")


def test_pnl_refresh_freezes_april_manual_517_and_does_not_repeat_it_in_june(tmp_path) -> None:
    source_module = load_module(
        "backend.app.services.pnl_source_service",
        "backend/app/services/pnl_source_service.py",
    )

    data_root = tmp_path / "data_input" / "pnl_517"
    data_root.mkdir(parents=True, exist_ok=True)
    headers = [
        "账务日期",
        "会计事件",
    ...
    april_book.save(data_root / "非标517-20260101-0430.xlsx")

    june_book = Workbook()
    june_sheet = june_book.active
    june_sheet.append(headers)
    june_sheet.append(
        [
            "2026-04-30",
            "投资收益",
    ...
    june_book.save(data_root / "非标517-20260101-0630(1).xlsx")

    april_refresh = source_module.load_latest_pnl_refresh_input(
        governance_dir=tmp_path / "governance",
        data_root=tmp_path / "data_input",
        report_date="2026-04-30",
    )
    june_refresh = source_module.load_latest_pnl_refresh_input(
        governance_dir=tmp_path / "governance",
        data_root=tmp_path / "data_input",
        report_date="2026-06-30",
    )

    assert [row["asset_code"] for row in april_refresh.nonstd_rows_by_type["517"]] == [
        "XS3271964481"
    ]
    assert april_refresh.nonstd_rows_by_type["517"][0]["raw_amount"] == source_module.Decimal(
        "154258000"
    )
    assert [row["asset_code"] for row in june_refresh.nonstd_rows_by_type["517"]] == [
        "JUNE-ONLY"
    ]

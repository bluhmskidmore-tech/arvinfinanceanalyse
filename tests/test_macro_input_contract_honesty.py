...
def test_m0041653_candidates_prefer_populated_choice_before_legacy_fallback() -> None:
    ordered = list(system_sources._LEGACY_ALIAS_CANDIDATES["m0041653"])
    candidates = _candidate_aliases("M0041653")

    assert ordered[0] == "EMM00088132"
    assert "EMM00088132" in ordered
    assert ordered.index("EMM00088132") < ordered.index("legacy.wind_market_db.reverse_repo_7d")

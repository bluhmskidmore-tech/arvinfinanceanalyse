...
// --- Extra: top-20 style-prop offenders ---
const sorted = [...pageStyleCounts.entries()].sort((a, b) => b[1] - a[1]);
console.log("\n=== Top-20 TSX files by style= count ===");
for (const [file, count] of sorted.slice(0, 20)) {
  console.log(`  ${count}\t${file}`);
}

if (failures.length > 0) {
  console.error("\nFrontend debt audit FAILED. Current debt may remain, but this change grows it.");

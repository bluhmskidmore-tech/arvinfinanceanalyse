...
describe("createApiClient", () => {
  ...
  it("keeps mock manual-adjustment current state reduced while honoring the audit query contract", async () => {
    const client = createApiClient({ mode: "mock" });
    ...
    const payload = await client.getProductCategoryManualAdjustments("2026-02-28", {
      ...
      eventSortDir: "asc",
      createdAtFrom: "2026-04-10T09:35:00Z",
      createdAtTo: "2026-04-10T09:45:00Z",
    });
  ...
  it("uses real mode to fetch product-category manual adjustments", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:8000/ui/pnl/product-category/manual-adjustments?report_date=2026-02-28&current_sort_field=created_at&current_sort_dir=desc&event_sort_field=created_at&event_sort_dir=desc",
      expect.objectContaining({
        headers: expect.objectContaining({
          Accept: "application/json",
        }),
  ...
  it("uses real mode to fetch filtered and paginated product-category manual adjustments", async () => {
    ...
    await client.getProductCategoryManualAdjustments("2026-02-28", {
      ...
      eventSortField: "event_type",
      eventSortDir: "asc",
      createdAtFrom: "2026-04-10T08:00:00Z",
      createdAtTo: "2026-04-10T10:00:00Z",
      adjustmentLimit: 5,
      adjustmentOffset: 10,
      limit: 10,
      offset: 20,
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:8000/ui/pnl/product-category/manual-adjustments?report_date=2026-02-28&current_sort_field=account_code&current_sort_dir=asc&event_sort_field=event_type&event_sort_dir=asc&adjustment_id=pca-1&adjustment_id_exact=true&account_code=5140&approval_status=approved&event_type=edited&created_at_from=2026-04-10T08%3A00%3A00Z&created_at_to=2026-04-10T10%3A00%3A00Z&adjustment_limit=5&adjustment_offset=10&limit=10&offset=20",
      expect.objectContaining({
        headers: expect.objectContaining({
          Accept: "application/json",
        }),
  ...
  it("uses real mode to export filtered product-category manual adjustments as csv", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:8000/ui/pnl/product-category/manual-adjustments/export?report_date=2026-02-28&current_sort_field=account_code&current_sort_dir=asc&event_sort_field=event_type&event_sort_dir=asc&adjustment_id=pca-1&adjustment_id_exact=true&account_code=5140&approval_status=approved&event_type=edited&created_at_from=2026-04-10T08%3A00%3A00Z&created_at_to=2026-04-10T10%3A00%3A00Z",
      expect.objectContaining({
        headers: expect.objectContaining({

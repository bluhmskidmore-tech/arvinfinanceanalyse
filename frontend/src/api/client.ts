...
import type {
  ...
  ProductCategoryManualAdjustmentSortDir,
...
const DEFAULT_MANUAL_ADJUSTMENT_SORT_FIELD = "created_at";
const DEFAULT_MANUAL_ADJUSTMENT_SORT_DIR: ProductCategoryManualAdjustmentSortDir =
  "desc";
...
function normalizeManualAdjustmentQuery(
  options: ProductCategoryManualAdjustmentQuery = {},
): ProductCategoryManualAdjustmentQuery {
  return {
    ...options,
    adjustmentId: normalizeQueryValue(options.adjustmentId) ?? "",
    accountCode: normalizeQueryValue(options.accountCode) ?? "",
    approvalStatus: normalizeQueryValue(options.approvalStatus) ?? "",
    eventType: normalizeQueryValue(options.eventType) ?? "",
    currentSortField:
      normalizeQueryValue(options.currentSortField) ??
      DEFAULT_MANUAL_ADJUSTMENT_SORT_FIELD,
    currentSortDir: options.currentSortDir ?? DEFAULT_MANUAL_ADJUSTMENT_SORT_DIR,
    eventSortField:
      normalizeQueryValue(options.eventSortField) ??
      DEFAULT_MANUAL_ADJUSTMENT_SORT_FIELD,
    eventSortDir: options.eventSortDir ?? DEFAULT_MANUAL_ADJUSTMENT_SORT_DIR,
    createdAtFrom: normalizeQueryValue(options.createdAtFrom) ?? "",
    createdAtTo: normalizeQueryValue(options.createdAtTo) ?? "",
  };
}

function compareManualAdjustmentValues(
  left: string | number | null | undefined,
  right: string | number | null | undefined,
  direction: ProductCategoryManualAdjustmentSortDir,
) {
...
function sortManualAdjustments(
  records: ProductCategoryManualAdjustmentPayload[],
  field: string,
  direction: ProductCategoryManualAdjustmentSortDir,
) {
  return [...records].sort((left, right) =>
    compareManualAdjustmentValues(
      left[field as keyof ProductCategoryManualAdjustmentPayload] as
        | string
        | number
        | null
        | undefined,
      right[field as keyof ProductCategoryManualAdjustmentPayload] as
        | string
        | number
        | null
        | undefined,
...
>(records: T[], options: ProductCategoryManualAdjustmentQuery = {}): T[] {
  const normalizedOptions = normalizeManualAdjustmentQuery(options);
  const adjustmentId = normalizedOptions.adjustmentId?.trim() ?? "";
  const adjustmentIdExact = options.adjustmentIdExact ?? false;
  const accountCode = normalizedOptions.accountCode?.trim() ?? "";
  const approvalStatus = normalizedOptions.approvalStatus?.trim() ?? "";
  const eventType = normalizedOptions.eventType?.trim() ?? "";
  const createdAtFrom = normalizedOptions.createdAtFrom?.trim() ?? "";
  const createdAtTo = normalizedOptions.createdAtTo?.trim() ?? "";
...
function buildManualAdjustmentSearchParams(
  ...
  const params = new URLSearchParams({
    ...
    current_sort_field:
      normalizedOptions.currentSortField ?? DEFAULT_MANUAL_ADJUSTMENT_SORT_FIELD,
    current_sort_dir:
      normalizedOptions.currentSortDir ?? DEFAULT_MANUAL_ADJUSTMENT_SORT_DIR,
    event_sort_field:
      normalizedOptions.eventSortField ?? DEFAULT_MANUAL_ADJUSTMENT_SORT_FIELD,
    event_sort_dir:
      normalizedOptions.eventSortDir ?? DEFAULT_MANUAL_ADJUSTMENT_SORT_DIR,
  });
  if (normalizedOptions.adjustmentId?.trim()) {
    params.set("adjustment_id", normalizedOptions.adjustmentId.trim());
  }
  if (normalizedOptions.adjustmentIdExact) {
    params.set("adjustment_id_exact", "true");
  }
  if (normalizedOptions.accountCode?.trim()) {
    params.set("account_code", normalizedOptions.accountCode.trim());
  }
  if (normalizedOptions.approvalStatus?.trim()) {
    params.set("approval_status", normalizedOptions.approvalStatus.trim());
  }
  if (normalizedOptions.eventType?.trim()) {
    params.set("event_type", normalizedOptions.eventType.trim());
  }
  if (normalizedOptions.createdAtFrom?.trim()) {
    params.set("created_at_from", normalizedOptions.createdAtFrom.trim());
  }
  if (normalizedOptions.createdAtTo?.trim()) {
    params.set("created_at_to", normalizedOptions.createdAtTo.trim());
  }
  if (normalizedOptions.adjustmentLimit !== undefined) {
    params.set("adjustment_limit", String(normalizedOptions.adjustmentLimit));
  }
  if (normalizedOptions.adjustmentOffset !== undefined) {
    params.set("adjustment_offset", String(normalizedOptions.adjustmentOffset));
  }
  ...
  const mockClient: ApiClient = {
    ...
    async getProductCategoryManualAdjustments(reportDate, options = {}) {
      ...
      const events = sortManualAdjustments(
        filterManualAdjustments(allEvents, normalizedOptions),
        normalizedOptions.eventSortField ?? DEFAULT_MANUAL_ADJUSTMENT_SORT_FIELD,
        normalizedOptions.eventSortDir ?? DEFAULT_MANUAL_ADJUSTMENT_SORT_DIR,
      );
      const limit = normalizedOptions.limit ?? 20;
      const offset = normalizedOptions.offset ?? 0;
      const adjustmentLimit = normalizedOptions.adjustmentLimit ?? 20;
      const adjustmentOffset = normalizedOptions.adjustmentOffset ?? 0;
      const pagedEvents = events.slice(offset, offset + limit);
      const adjustments = sortManualAdjustments(
        filterManualAdjustments(
          reduceLatestManualAdjustments(allEvents),
          normalizedOptions,
        ),
        normalizedOptions.currentSortField ?? DEFAULT_MANUAL_ADJUSTMENT_SORT_FIELD,
        normalizedOptions.currentSortDir ?? DEFAULT_MANUAL_ADJUSTMENT_SORT_DIR,

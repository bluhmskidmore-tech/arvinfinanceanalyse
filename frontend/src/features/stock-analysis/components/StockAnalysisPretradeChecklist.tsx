 ...
 * 清单不可用(404/失败 → null)或候选为空(checklist_status=empty/items 空)时
 * 整面隐藏(render null)，不留空壳。契约全 optional，缺失字段一律 EM_DASH。
...
export type StockAnalysisPretradeChecklistProps = {
  ...
  loadChecklist?: () => Promise<PretradeChecklistPayload | null>;
};
...
function statusPresentation(status: string | null | undefined) {
  ...
  return STATUS_PRESENTATION[status] ?? { label: status, tone: "neutral" as StatusTone };
}
...
function dataFlagLabels(item: PretradeChecklistItem): string[] {
...
function ChecklistRow({ item }: { item: PretradeChecklistItem }) {
  const status = statusPresentation(item.buyable_status);
  const flags = dataFlagLabels(item);
  return (
    <li
    ...
    >
      ...
      <span
        ...
        title={item.trade_status ?? undefined}
      >
        {status.label}
      </span>
...
export function StockAnalysisPretradeChecklist({ loadChecklist }: StockAnalysisPretradeChecklistProps) {
  ...
  useEffect(() => {
    ...
    load()
      .then((payload) => {
        if (cancelled) return;
        if (payload && (payload.items?.length ?? 0) > 0) {
    ...
    >
      ...
      <ul className="stock-analysis-pretrade__rows">
        {items.map((item, index) => (
          <ChecklistRow key={item.stock_code ?? index} item={item} />
        ))}
      </ul>

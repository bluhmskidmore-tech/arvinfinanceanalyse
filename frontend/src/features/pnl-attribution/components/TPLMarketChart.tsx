...
export function TPLMarketChart({ data }: Props) {
  const chartOption = useMemo<EChartsOption | null>(() => {
    ...
    const tpl = data.data_points.map((p) => p.tpl_fair_value_change / 100_000_000);
    const bp = data.data_points.map((p) => p.treasury_10y_change ?? 0);
    return {
  ...
  const corr = correlationLabel(data.correlation_coefficient);
      ...
      >
        <div style={{ ...cardStyle, padding: 16, background: corr.bg }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 700, color: corr.color }}>
            {data.correlation_coefficient !== null ? data.correlation_coefficient.toFixed(3) : "—"}
          </div>
        ...
        <div style={{ ...cardStyle, padding: 16 }}>
          ...
          <div
            style={{
              fontSize: 18,
              fontWeight: 700,
              color: data.total_tpl_fv_change >= 0 ? "#15803d" : "#b91c1c",
            }}
          >
            {formatYi(data.total_tpl_fv_change)}
          </div>
        </div>
        <div style={{ ...cardStyle, padding: 16 }}>
          ...
          <div
            style={{
              fontSize: 18,
              fontWeight: 700,
              color: (data.treasury_10y_total_change ?? 0) <= 0 ? "#15803d" : "#b91c1c",
            }}
          >
            {data.treasury_10y_total_change !== null
              ? `${data.treasury_10y_total_change >= 0 ? "+" : ""}${data.treasury_10y_total_change.toFixed(1)} BP`
              : "—"}
          </div>
        </div>
        <div style={{ ...cardStyle, padding: 16 }}>
              ...
              {data.data_points.map((point, idx) => (
                <tr key={idx} style={{ borderBottom: "1px solid #eef2f7" }}>
                  ...
                  <td style={{ textAlign: "right", padding: 10, fontVariantNumeric: "tabular-nums" }}>
                    {(point.tpl_scale / 100_000_000).toFixed(2)}
                  </td>
                  <td
                    style={{
                      textAlign: "right",
                      padding: 10,
                      color: point.tpl_fair_value_change >= 0 ? "#15803d" : "#b91c1c",
                    }}
                  >
                    {(point.tpl_fair_value_change / 100_000_000).toFixed(2)}
                  </td>
                  <td style={{ textAlign: "right", padding: 10 }}>
                    {point.treasury_10y !== null ? point.treasury_10y.toFixed(3) : "—"}
                  </td>
                  <td
                    style={{
                      textAlign: "right",
                      padding: 10,
                      color: (point.treasury_10y_change ?? 0) <= 0 ? "#15803d" : "#b91c1c",
                    }}
                  >
                    {point.treasury_10y_change !== null
                      ? `${point.treasury_10y_change >= 0 ? "+" : ""}${point.treasury_10y_change.toFixed(1)}`
                      : "—"}
                  </td>
                  <td style={{ textAlign: "right", padding: 10 }}>
                    {point.dr007 !== null ? point.dr007.toFixed(3) : "—"}
                  </td>
                </tr>
              ))}
            </tbody>

...
function correlationLedgerCellTone(value: string) {
...
function correlationLedgerCellClassName(value: string) {
  const tone = correlationLedgerCellTone(value);
  const parsed = Number(value);
  ...
  return [
    "cross-asset-drivers-page__tabular-cell",
    "cross-asset-linkage-heatmap-ledger__cell",
    `cross-asset-linkage-heatmap-ledger__cell--${tone}`,
    `cross-asset-linkage-heatmap-ledger__cell--${strength}`,
  ].join(" ");
}

function splitLinkageIndicator(value: string) {
  const [source, ...targetParts] = value.split("→");
  return {
...
const MARKET_TAPE_ITEMS = [
  { key: "cn_gov_10y", label: "CN10Y" },
  { key: "money_market_7d", label: "DR007" },
  { key: "financial_conditions", label: "CSI300" },
  { key: "brent", label: "Brent" },
  { key: "usdcny", label: "USD/CNY" },
  { key: "gov_spread", label: "中美利差" },
] as const;

function CrossAssetMarketTape({ kpis }: { kpis: ResolvedCrossAssetKpi[] }) {
    ...
    <div className="cross-asset-market-tape" data-testid="cross-asset-market-tape" role="list" aria-label="跨资产市场快讯">
      {MARKET_TAPE_ITEMS.map((item) => {
        ...
        return (
          <div
            key={item.key}
            className={`cross-asset-market-tape__item cross-asset-market-tape__item--${kpi?.changeTone ?? "default"}`}
            role="listitem"
          >
            <span>{item.label}</span>
            <strong>{kpi?.valueLabel ?? "待定"}</strong>
            <em>{kpi?.changeLabel ?? "待确认"}</em>
          </div>
        );
      })}
    </div>
...
function CrossAssetEvidenceTape({
  kpis,
...
function CrossAssetReviewQueue({
  statusFlags,
  latestMeta,
  linkageMeta,
  isLoading,
...
function CrossAssetReferenceToolbar({
  reportDate,
  onRefresh,
}: {
  reportDate: string;
...
function CrossAssetReferenceSummary({
  reportDate,
  conclusion,
  marketRegime,
  latestMeta,
  ...
  marketRegime: ReturnType<typeof identifyMarketRegime>;
  latestMeta: ResultMeta | undefined;
  linkageMeta: ResultMeta | undefined;
  statusFlags: CrossAssetStatusFlag[];
  isLoading: boolean;
}) {
  const summaryText =
    isLoading ? "正在加载联动分析…" : conclusion || "宏观环境偏松，利率下行空间，可适度拉长久期";
    ...
    <section className="cross-asset-reference-summary" data-testid="cross-asset-reference-summary">
      <section className="cross-asset-decision-header cross-asset-reference-summary__conclusion" data-testid="cross-asset-decision-header">
        ...
        >
          ...
          <h1 className="moss-page-v2-decision-hero__title" title={summaryText}>{headlineText}</h1>
...
function CrossAssetReferenceMarketTape({ kpis }: { kpis: ResolvedCrossAssetKpi[] }) {
...
function CrossAssetReferenceEvidenceMatrix({ kpis }: { kpis: ResolvedCrossAssetKpi[] }) {
...
function CrossAssetReferenceSourceAudit({
  reportDate,
  latestMeta,
  linkageMeta,
  statusFlags,
    ...
    <section className="cross-asset-reference-card cross-asset-reference-judgments" data-testid="cross-asset-research-views">
      ...
      <div className="cross-asset-reference-judgments__grid">
        {visibleCards.map((card) => {
          ...
          return (
            <article
              className="cross-asset-reference-judgment"
              data-testid={`cross-asset-research-card-${card.key}`}
              key={card.key}
            >
...
function CrossAssetReferenceLowerGrid({
  matrix,
  rows,
  cards,
}: {
...
function CrossAssetReferenceTrendStrip({ kpis }: { kpis: ResolvedCrossAssetKpi[] }) {
...
function CrossAssetReferenceDepthSummary({
  kpis,
  heatmapCount,
  transmissionRows,
  assetRows,
  ...
  const depthItems = [
    {
      ...
      label: "\u8bc1\u636e\u660e\u7ec6",
      value: `${kpis.length} \u9879`,
      meta: `${heatmapCount} \u7ec4\u76f8\u5173`,
    },
    {
      key: "transmission",
      label: "\u4f20\u5bfc\u8f74",
      value: `${readyAxisCount}/${transmissionRows.length}`,
      meta: "\u5df2\u53ef\u7528",
    },
    {
      key: "asset",
      label: "\u8d44\u4ea7\u5224\u65ad",
      value: `${readyAssetCount}/${assetRows.length}`,
      meta: `${pendingAssetInputCount} \u9879\u5f85\u63a5\u5165`,
    },
    {
      key: "market",
      label: "\u52a8\u91cf\u76f8\u5173",
      value: `${momentumRows.length} \u884c`,
      meta: `${matrixPairCount} \u5bf9\u7cfb\u6570`,
    },
    {
      key: "action",
      label: "\u884c\u52a8\u961f\u5217",
      value: `${candidateActions.length} \u9879`,
      meta: `${statusFlags.length} \u9879\u6821\u9a8c`,
    },
    {
      key: "review",
      label: "\u89c2\u5bdf\u590d\u6838",
      value: `${observationCount} \u9879`,
      meta: `${eventCount} \u4e8b\u4ef6 / ${watchCount} \u76d1\u63a7`,
    },
  ];

  return (
    <section
      className="cross-asset-reference-depth-summary"
      data-testid="cross-asset-reference-depth-summary"
      aria-label="\u4e0b\u65b9\u6570\u636e\u603b\u89c8"
    >
          ...
          <div className="cross-asset-data-status-strip__kv" data-testid="cross-asset-status-flags">
            {statusFlags.map((flag) =>
              crossAssetStatusFlagUsesAlertChip(flag) ? (
                <div
                  key={flag.id}
                  className="cross-asset-data-status-strip__flag cross-asset-data-status-strip__flag--alert"
                  title={flag.detail}
                >
...
function _LegacyCrossAssetDecisionHeader({
  reportDate,
  conclusion,
  isLoading,
  isMockMode,
  ...
  const conclusionText = isLoading
    ? "正在加载联动分析…"
    ...
    <header className="cross-asset-decision-header" data-testid="cross-asset-decision-header">
      <div className="cross-asset-decision-header__main">
        ...
        >
          ...
          <div className="moss-page-v2-decision-hero__conclusion">
            <span>今日传导结论：{conclusionText}</span>
          </div>
        </section>
      </div>
...
function _LegacyCrossAssetMarketStateStrip({
  kpis,
  envTags,
}: {
  kpis: ResolvedCrossAssetKpi[];
  ...
  const kpisByKey = new Map(kpis.map((kpi) => [kpi.key, kpi]));
  const radarMetrics = [
    { key: "cn_gov_10y", label: "10Y国债" },
    { key: "money_market_7d", label: "银拆(7D)" },
    { key: "gov_spread", label: "中美利差" },
    { key: "financial_conditions", label: "权益风险" },
  ];
  ...
  return (
    <section className="cross-asset-market-state-strip" data-testid="cross-asset-market-state-strip" aria-labelledby="cross-asset-market-state-strip-title">
      ...
      <div className="cross-asset-market-state-strip__tags">
        {[
          { tone: "主导", value: envTags.primary },
          { tone: "次要", value: envTags.secondary },
          { tone: "风格", value: envTags.style },
        ].map((tag) => (
          <div key={tag.tone} className="cross-asset-market-state-strip__tag-row">
            <span className="cross-asset-market-state-strip__tag-label">{tag.tone}</span>
            <span className="cross-asset-market-state-strip__tag-value">{tag.value}</span>
          </div>
        ))}
      </div>
...
const TRANSMISSION_PATH_STEPS = [
  { key: "rates", label: "利率", sourceAxis: "global_rates" },
  { key: "credit", label: "信用/NCD", sourceAxis: "liquidity" },
  { key: "equity", label: "权益", sourceAxis: "mega_cap_equities" },
  { key: "commodities", label: "商品", sourceAxis: "commodities_inflation" },
  { key: "fx", label: "汇率", sourceAxis: "equity_bond_spread" },
  { key: "bond_action", label: "债券组合动作", sourceAxis: "global_rates" },
] as const;

function CrossAssetTransmissionMap({ rows }: { rows: CrossAssetTransmissionAxisRow[] }) {
  const rowsByAxis = useMemo(() => new Map(rows.map((row) => [row.axisKey, row])), [rows]);
    ...
    <section className="cross-asset-transmission-map" data-testid="cross-asset-transmission-map" aria-label="跨资产传导链">
      ...
      <ol className="cross-asset-transmission-map__steps">
        {TRANSMISSION_PATH_STEPS.map((step, index) => {
          const axis = rowsByAxis.get(step.sourceAxis);
          return (
            <li key={step.key} className={`cross-asset-transmission-map__step cross-asset-transmission-map__step--${axis?.status ?? "pending_signal"}`}>
              <span className="cross-asset-transmission-map__index">{String(index + 1).padStart(2, "0")}</span>
              <strong>{step.label}</strong>
              <small>{axis?.stanceLabel ?? "待信号"}</small>
            </li>
          );
        })}
      </ol>
...
function _LegacyCrossAssetTransmissionCanvas({
  rows,
  researchViewCards,
}: {
  rows: CrossAssetTransmissionAxisRow[];
...
function NcdProxyEvidencePanel({
  ...
  return (
    <details
      ...
      className={
        isProxyNotMatrix
          ? "cross-asset-ncd-proxy cross-asset-ncd-proxy--warning"
          : "cross-asset-ncd-proxy"
      }
    >
      <summary>
        <span className="cross-asset-ncd-proxy__summary-main">
          <strong className="cross-asset-ncd-proxy__title">NCD / 资金代理</strong>
          <span className="cross-asset-ncd-proxy__label">{evidence.proxyLabel}</span>
        </span>
          ...
          >
            ...
            <div className="cross-asset-drivers-page__chip-row">
              ...
              {row.impactedViews.length > 0 ? (
                <span className="cross-asset-transmission-axes__impact">
                  影响：{formatImpactedViewsForDisplay(row.impactedViews)}
                </span>
              ) : null}
            </div>
...
function buildCorrelationSummaryCards(matrix: CorrelationMatrix): CorrelationSummaryCard[] {
  const pairs = matrix.cells.flatMap((row, rowIndex) =>
  ...
  const strongestPositive =
    pairs.slice().sort((left, right) => (right.value ?? -Infinity) - (left.value ?? -Infinity))[0];
  const strongestNegative =
    pairs.slice().sort((left, right) => (left.value ?? Infinity) - (right.value ?? Infinity))[0];
  const strongestAbsolute =
    pairs.slice().sort((left, right) => Math.abs(right.value ?? 0) - Math.abs(left.value ?? 0))[0];
...
function VolatilityClusteringPanel({ alert }: { alert: VolatilityAlert }) {
  ...
  const foldedAssetsTitle = foldedAssets.map((asset) => `${asset.label} ${asset.volRatio.toFixed(2)}x`).join(" / ");
      ...
      {alert.assets.length > 0 ? (
        <div className="ca-vol-alert__bars">
          ...
          {foldedAssets.length > 0 ? (
            <div
              className="ca-vol-alert__bar-item ca-vol-alert__bar-item--folded"
              data-testid="cross-asset-vol-folded-assets"
              title={foldedAssetsTitle}
            >
              <div className="ca-vol-alert__bar-label">{`其余 ${foldedAssets.length} 项`}</div>
              <div className="ca-vol-alert__bar-value">常规波动</div>
            </div>
          ) : null}
        </div>
      ) : null}
...
function DriverWaterfallPanel({
  ...
  const gapRows = factorRows.filter(
    ({ evidence }) => evidence.status === "missing" || evidence.status === "neutral",
  );
  ...
  const chartHeight = compactGapMode ? 72 : 120; // px available for bars
    ...
    <section className="ca-waterfall" data-testid="cross-asset-driver-waterfall">
      ...
      <div className="ca-waterfall__decision" data-testid="cross-asset-driver-waterfall-decision">
        <div className={`ca-waterfall__decision-card ca-waterfall__decision-card--${netTone}`}>
          ...
          <small>{netTone === "positive" ? "偏利好债市" : netTone === "negative" ? "偏利空债市" : "方向中性"}</small>
        </div>
        ...
        {gapRows.length > 0 ? (
          <div className={gapStripClassName} data-testid="cross-asset-driver-waterfall-gap-strip">
            {gapRows.map(({ bar, evidence }) => (
              <span
                key={`gap-${bar.key}`}
                className={`ca-waterfall__evidence-chip ca-waterfall__evidence-chip--${evidence.status}`}

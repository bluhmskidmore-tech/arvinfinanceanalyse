...
DEFAULT_TEMPLATE = ROOT / "docs" / "pnl" / "stock-analysis-business-owner-approval-template.md"
EXPECTED_SIGNOFF_PACKET = "docs/pnl/stock-analysis-sign-off-packet.md"
EXPECTED_GOVERNANCE_AUDIT_PACKET = "docs/pnl/stock-analysis-governance-audit-packet.md"
EXPECTED_OWNER_EVIDENCE_PACKET = "docs/pnl/stock-analysis-owner-evidence-packet.md"
...
ACTION_ITEM_DEFINITIONS = {
    ...
    "golden_sample_review": {
        "template_field": "- Golden sample `GS-STOCK-ANALYSIS-OBS-A` reviewed",
        "required_value": "yes",
    },
...
def _project_root_for_template(template_path: Path) -> Path:
    ...
    if resolved.parent.name == "pnl" and resolved.parent.parent.name == "docs":
...
def _review_packet_status(
    ...
    *,
    ...
    if _plain_value(text, label) != expected_path:
        return "invalid"
    if not (_project_root_for_template(template_path) / expected_path).is_file():
...
def _approval_blockers(
    ...
    *,
    ...
    packet_fields = {
        "Reviewed sign-off packet": ("reviewed_signoff_packet", EXPECTED_SIGNOFF_PACKET),
        "Reviewed governance audit packet": (
            "reviewed_governance_audit_packet",
            EXPECTED_GOVERNANCE_AUDIT_PACKET,
        ),
        "Reviewed owner evidence packet": (
            "reviewed_owner_evidence_packet",
    ...
    review_fields = {
        ...
        "- Golden sample `GS-STOCK-ANALYSIS-OBS-A` reviewed": "golden_sample_review",
        ...
        "- No-trading-instruction boundary accepted": "not_trading_instruction_review",
    }
...
def _approval_field_status(
    ...
    *,
    ...
    return {
        ...
        "business_owner_signature": "valid" if _plain_value_is_filled(text, "Business owner signature") else "missing",
        "reviewed_signoff_packet": _review_packet_status(
            text,
            "Reviewed sign-off packet",
            EXPECTED_SIGNOFF_PACKET,
            template_path=template_path,
        ),
        "reviewed_governance_audit_packet": _review_packet_status(
            text,
            "Reviewed governance audit packet",
            EXPECTED_GOVERNANCE_AUDIT_PACKET,
            template_path=template_path,
        ),
        "reviewed_owner_evidence_packet": _review_packet_status(
            text,
            "Reviewed owner evidence packet",
            EXPECTED_OWNER_EVIDENCE_PACKET,
            template_path=template_path,
        ...
        "golden_sample_review": _review_status(text, "- Golden sample `GS-STOCK-ANALYSIS-OBS-A` reviewed"),
        ...
        "not_trading_instruction_review": _review_status(text, "- No-trading-instruction boundary accepted"),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Read the stock-analysis business-owner approval template and report its current status.",
    )
    parser.add_argument(
        "--template-path",
        type=Path,

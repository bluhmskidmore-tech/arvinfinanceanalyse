...
DEFAULT_TEMPLATE = ROOT / "docs" / "pnl" / "bond-analysis-business-owner-approval-template.md"
...
def _approval_action_items(
    ...
    for blocker in remaining_blockers:
        if blocker in {"business_owner_approval", "candidate_boundary_acceptance"}:
            continue
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Read the bond-analysis business-owner approval template and report its current status.",
    )
    parser.add_argument(
        "--template-path",
        type=Path,

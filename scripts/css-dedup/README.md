...
- 非 module 的全局 CSS：config 置 `"evidenceMode": "literal"`（className 字面量提取）；
  开工前必须跑 `node precheck-global.js <cssPath>`——@import 必须为零（否则文件连续性假设
  失效，禁止合并），跨文件同名类与多入口副作用导入属于披露项（文件内合并不改变跨文件
  相对顺序，各拷贝内部各自收敛，正确性不受影响，但审阅者需知情）。

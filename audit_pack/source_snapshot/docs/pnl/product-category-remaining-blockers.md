...
1. **Evidence baseline + Unit 10 scenario assertions:** create a compact readiness note and extend the existing golden assertion text for the already-documented companion scenario probe. Scope: `docs/pnl/product-category-development-data-readiness.md`, `tests/golden_samples/GS-PROD-CAT-PNL-A/assertions.md`. Do not add `metric_id` bindings or a second full scenario sample without approval.
2. **Unit 5 / Unit 7 / Unit 10 documentation links:** add canonical-surface/cross-link notes for main vs audit lifecycle and checklist-to-truth-contract navigation. Scope: `product-category-page-truth-contract.md`, this file, and checklist references only. Do not change lifecycle product policy.
3. **Unit 3 / Unit 8 state matrix skeleton:** add a stale/fallback/refresh matrix template with known cells filled from current tests and unknown cells marked decision-required. Scope: `docs/pnl/product-category-page-truth-contract.md` or a linked readiness note. Do not invent final banner text, timeout copy, fallback-date rules, or `as_of_date` semantics.
